...
`lcm_grep` and `lcm_recall` accept `exclude_current_session=false` and
`exclude_session_ids=[]`. The defaults preserve existing results. When enabled,
the tools remove matching session ids before candidate caps, fusion, and final ranking. Excluding
the only session selected by `session_scope='current'` or `'session'` returns a
normal bounded response with no results.
