...
session's stored project identity. It returns an error if that session has no
project metadata instead of guessing from the plugin process's working directory.
An explicit, non-empty string `project_id` takes precedence over
`project_scope`; it must match the stored identity exactly.
